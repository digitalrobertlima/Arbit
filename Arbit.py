...
def fetch_data():
    ...
    while not successful:
        try:
            ...
            print('[ERROR] Something happened while fetching the order book: \n' + e)

        print('Trying again in ' + (2 ** attempt) + ' seconds...')
    ...
    for bExchange, bPrice in exchanges.items():
        for sExchange, sPrice in exchanges.items():
            ...
            if (totalSPrice > totalBPrice) and (profit > 0.1):
                ...
                # TODO: Notificates only for foxbit as selling exchange to debug
                # In the future it should alternate between exchanges
                if sExchange == 'FOX':
