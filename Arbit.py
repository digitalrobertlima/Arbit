    ...
    for bExchange, bPrice in exchanges.items():
        for sExchange, sPrice in exchanges.items():
            ...
            if (totalSPrice > totalBPrice) and profit > 0.1:
                ...
                print('Placing sell order in ' + sExchange + '...')
                if sExchange == 'MBT':
                    mbtOrder = mbtTrade.place_sell_order(quantity=0.001, limit=sPrice['bid'])
                    print(mbtOrder.data)
                elif sExchange == 'FOX':
                    fox.place_sell_order(quantity=0.001, limit=sPrice['bid'], auth=foxAuth)

                print('Placing buy order in ' + bExchange + '...')
                if bExchange == 'MBT':
                    mbtOrder = mbtTrade.place_buy_order(quantity=0.001, limit=bPrice['ask'])
                    print(mbtOrder.data)
                elif bExchange == 'FOX':
                    fox.place_buy_order(quantity=0.001, limit=bPrice['ask'], auth=foxAuth)

                print("We're done for now, take a look to see if it worked!")
                sys.exit()
